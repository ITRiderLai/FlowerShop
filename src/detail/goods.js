...
class Goods extends Component {
    ...
    renderFoot(){
        return(
            <View style={{position:"absolute",bottom:0,flexDirection:'row'}}>
                    <TouchableNativeFeedback background={TouchableNativeFeedback.SelectableBackground()}  onPress={()=> this.goCart() }>
    ...
    // 加入购物车
    addCartFn() {
        let goods = Object.assign({}, this.state)
        this.props.cart.addGoods(goods)
        alert('加入成功')
    }
    ...
    render() {
        return (
            <View style={{flex:1,width:width,height:height,justifyContent:'center',alignItems:'center',backgroundColor:'white'}}>
                <ScrollView>
                   <Image style={styles.goodstyle} source={{uri:this.state.headerimg}}/>
                    ...
                    <View style={styles.pricebox}>
                        <Text style={{paddingLeft:width*0.05,fontSize:width*0.08,color:'#fa6f57'}}>￥{this.state.price}</Text>
                        <Text style={{paddingLeft:width*0.05,fontSize:15}}>2.8折</Text>
                        <Text style={{textDecorationLine:'line-through',paddingLeft:width*0.05,fontSize:15}}>￥389.00</Text>
                    </View> 
                </ScrollView>
                {this.renderFoot()}
                {this.renderModal()}
